...
export class RegisterComponent {
  ...
  /**
   * Submits the registration form. Supabase signUp errors (e.g. duplicate email)
   * are shown in a top-centred snackbar. On success the user is redirected to the
   * homepage; Supabase automatically signs them in and the auth state listener
   * in AuthService picks up the new session without any extra handling here.
   */
  onSubmit(): void {
    ...
    this.authService.register(email, username, password)
      ...
      .subscribe(authResult => {
        if (authResult.error) {
          ...
          this.notificationService.showSuccess(this.ts.t('register.success'));
